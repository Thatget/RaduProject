...
class Framework extends \Magento\Framework\App\Helper\AbstractHelper {
	...
	public function generatePreview($data) {
		...
		$result = str_replace('</body>', $content . '</body>', $blankPage);
	...
	public function preloadPreviewPage() {
		$blankPage = file_get_contents($this->_storeManager->getStore()->getBaseUrl() . 'nwdthemes_revslider/index');
		$this->update_option('preview_blank_page', $blankPage);
	}
	...
	protected function _makeImagesRelative($data) {
		...
		} elseif (is_string($data) && $data !== $this->getImagesHelper()->imageFile($data)) {
