...
namespace Bss\MageWorxMultiFees\Rewrite\Magento\Customer\Model;
...
class EmailNotification implements \Magento\Customer\Model\EmailNotificationInterface
{
    ...
    private function emailAndPasswordChanged(CustomerInterface $customer, $email)
    {
        ...
        $this->sendEmailTemplate(
            ...
            ['customer' => $customerEmailData, 'store' => $this->storeManager->getStore($storeId), "customer_password"=>'1231213'],
            $storeId,
            $email
        );
    }
    ...
    public function newAccount(
        ...
        $customerEmailData = $this->getFullCustomerObject($customer);
        $password = $this->_registry->registry('customer_password');
        $this->_registry->unregister('customer_password');
        $this->sendEmailTemplate(
            $customer,
            $types[$type],
